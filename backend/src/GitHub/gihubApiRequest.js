...
const getCodeContent = async (url) => {
  try {
    ...
    let decodedContent = Buffer.from(response.data.content, "base64").toString(
      "utf-8"
    );
    const fileExtension = url.split("/").slice(-1)[0].split(".").pop();
    if (decodedContent.includes("using System") && fileExtension === "cs") {
      decodedContent = decodedContent.slice(
        decodedContent.indexOf("using System")
      );
    }
  ...
  try {
    ...
    for (const x of response.data) {
      if (x.type === "dir") {
        ...
        x.type === "file" &&
        correctFileTypes().includes(x.name.split(".")[1])
      ) {
        if (!data?.files) data.files = [];
        data.files.push({
          fileName: x.name,
          filePath: x.html_url.replace(process.env.URL_ADDRESS, ""),
        });
      }
    }
